...
class DenominationControllerTest {
    ...
    void testCalculateEndpoint() throws Exception {
        ...
        DenominationResponse mockResponse = new DenominationResponse(4532L, breakdown, difference);
        ...
        mockMvc.perform(
                post("/api/denomination/v1").contentType(MediaType.APPLICATION_JSON).content(json))
                .andExpect(status().isOk()).andExpect(jsonPath("$.newValue").value(4532))
